...
function Teamcity(runner, options) {
  ...
  useStdError
    ? (useStdError = useStdError.toLowerCase() === "true")
    : (useStdError = false)
  recordHookFailures
    ? (recordHookFailures = recordHookFailures.toLowerCase() === "true")
    : (recordHookFailures = false)
  actualVsExpected
    ? (actualVsExpected = actualVsExpected.toLowerCase() === "true")
    : (actualVsExpected = false)
  Base.call(this, runner)
  let stats = this.stats
  const topLevelSuite =
    reporterOptions.topLevelSuite ||
    process.env["MOCHA_TEAMCITY_TOP_LEVEL_SUITE"]
  ...
  runner.on("fail", function(test, err) {
    if (actualVsExpected && err.actual && err.expected) {
      ...
      if (useStdError) {
        logError(
          formatMessage(TEST_FAILED, test.title, err.message, err.stack, flowId)
        )
      } else {
        log(
          formatMessage(TEST_FAILED, test.title, err.message, err.stack, flowId)
  ...
  runner.on("end", function() {
    ...
    if (topLevelSuite) {
      isNil(duration)
        ? log(formatMessage(SUITE_END_NO_DURATION, topLevelSuite, flowId))
        : log(formatMessage(SUITE_END, topLevelSuite, duration, flowId))
    }
  })
}
