...
class TeamCityErrorPlugin {
  ...
  apply(compiler) {
    compiler.hooks.done.tap("TeamCityErrorPlugin", stats => {
      const warningsFilters = this.warningsFilter(
        stats.compilation.options.stats.warningsFilter
      )
        ...
        if (error instanceof ModuleNotFoundError) {
          error.dependencies.forEach(dependency => {
            console[type](
              formatMessage(
                TYPED_MESSAGE,
                `Module '${dependency.request}' not found`,
                type
              )
            )
          })
      ...
      stats.compilation.warnings
        .filter(warning =>
          warningsFilters.every(warningFilter => warningFilter(warning))
        )
        .forEach(warning => {
          const type = "warn"
