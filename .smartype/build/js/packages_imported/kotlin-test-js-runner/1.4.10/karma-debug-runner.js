...
function configureBrowsers(config) {
  ...
  let browser = newBrowsers.find(browserName =>
    isDebuggableBrowser(browserName, config)
  )
...
function isDebuggableBrowser(browserName, config) {
  if (
    ["ChromeHeadless", "ChromeCanaryHeadless", "ChromiumHeadless"].includes(
      browserName
    )
  ) {
    return true
  ...
  const value = flags.find(
    flag => typeof flag === "string" && flag.indexOf(prefix) === 0
  )
  if (value == null) {
    return false
  }
