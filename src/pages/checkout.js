...
const Checkout = ({ context }) => {
  ...
  const handleSubmit = async event => {
    ...
    var fullName = name.toString().split(" ")
    var identityCallback = function(result) {
      if (result.getUser()) {
        const currentUser = result.getUser()
        currentUser.setUserAttribute("$FirstName", fullName[0])
        currentUser.setUserAttribute("$LastName", fullName[1])
        currentUser.setUserAttribute("$Address", street)
        currentUser.setUserAttribute("$City", city)
        currentUser.setUserAttribute("$State", state)
        currentUser.setUserAttribute("$Zip", postal_code)
      }
    }
